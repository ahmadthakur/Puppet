...
module.exports = {
    ...
    async execute(interaction) {
        //use weather api to get weather info
        const city = interaction.options.getString('city');
        //join two or more words together
        const joinedCity = city.split(' ').join('%20');
        //get the weather info of the city
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${joinedCity}&appid=`+weatherKey+`&units=metric`)
